...
**Important:**
Before running `ai-engine-bootstrap.sh`, ensure the LXC container (ID 101, name "ai-engine") is created on the Prox01 host with its root disk and the `/srv/ai/models` bind mount both located on the `RaidZ1-6TB` pool (not `rpool`).
Storing models or large AI data on `rpool` can fill the root pool and crash the host. Always use the large data pool for model storage and container disks.
